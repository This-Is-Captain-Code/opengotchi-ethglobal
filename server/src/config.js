...
export const config = {
  ...
  // Blink (blink.cash) stablecoin deposit SDK — fund the pet wallets with USDC.
  // Register blink_public.pem with Blink to get a merchantId.
  blink: {
    merchantId: process.env.BLINK_MERCHANT_ID || '',
    privateKeyPem: (process.env.BLINK_PRIVATE_KEY || '').replace(/\\n/g, '\n'),
    environment: process.env.BLINK_ENV || 'production',
  },
  devices,
};
